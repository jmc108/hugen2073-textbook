...
We have three tables. `d` has some individual-level data on 1000 Genomes Project subjects, and `d_summary` and `d_summary2` are  population-level summary tables. All three are previewed below.
...
The data are in `cv.csv`, which you should download from the class book/website and read in. Apply `head` to see the variables. We also pre-process a little.
