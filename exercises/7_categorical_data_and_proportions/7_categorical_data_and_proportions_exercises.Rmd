...
# Read data
gtex <- read_tsv(
  "https://jmc108.github.io/hugen2073-textbook/exercises/7_categorical_data_and_proportions/GTEx_Analysis_2017-06-05_v8_RNASeQCv1.1.9_gene_median_tpm.gct.gz",
  skip = 2
)
...
msig_hallmark <- read.csv("https://jmc108.github.io/hugen2073-textbook/exercises/7_categorical_data_and_proportions/msig_hallmark.csv")
msig_hallmark_pairs <- read.csv("https://jmc108.github.io/hugen2073-textbook/exercises/7_categorical_data_and_proportions/msig_hallmark_pairs.csv")
...
- Need to provide an example of this (though it's covered in a previous exercise)

- This chunk is garbage:
...
Here's another upset plot example that uses non-genetic data. This example comes from the creator(s) of the package.
...
- How big was the universe of genes? `N=?` (Pretend `N`=100, which is unrealistically tiny.)
...
geneset <- paste0("genes",7:56)
ggvenn(list("DE genes (n=8)"=degenes, "Gene set (K=50)"=geneset)) + ggtitle("Universe size=100?")
```
...
First, there are this many ways to pick `n=8` genes from a "universe" of size `N=100` genes: 

$$
\binom{N}{n} = \frac{N!}{n!(N-n)!} = \frac{100!}{8!(92)!}
$$

How many ways are to pick out exactly `k=2` pathway genes in our "sample" of size `n=8`?

This is equivalent to picking `k=2` of the `K=50` pathway genes AND picking `n - k = 8 - 2 = 6` non-pathway genes out of the `N - K = 100 - 50 = 50` total non-pathway genes in the "universe".

The number of ways to do that is: 

$$
\binom{K}{k} * \binom{N-K}{n-k} = \binom{50}{2} * \binom{50}{6} = \frac{50!}{2!(48)!} * \frac{50!}{6!(44)!} 
$$
So the probability of getting an overlap of *exactly* `k=2` is then

$$
\frac{\frac{50!}{2!(48)!} * \frac{50!}{6!(44)!} }{\frac{100!}{8!(92)!}}
$$
We can calculate that in R.

```{r}
(factorial(50)/(factorial(2)*factorial(48))) * (factorial(50)/(factorial(6)*factorial(44))) / (factorial(100)/(factorial(8)*factorial(92)))
```

We can verify this with `dhyper`, the probability density function for the hypergeometric distribution.

```{r}
# Note that the m, n, and k parameters have different meanings here
dhyper(x=2, m=50, n=50, k=8)
```

Note we are not quite done. A p-value would be the probability of getting results *at least* this extreme, i.e., the probability that `k` is at least 2.

Since `k` could only reach `n=8` at most, that is:

$$
\sum_{k=2}^{8} \frac{\frac{50!}{k!(50-k)!} * \frac{50!}{(8-k)!(42-k)!} }{\frac{100!}{8!(92)!}}
$$

We can calculate like this:

```{r}
sum(dhyper(x=2:8, m=50, n=50, k=8))
```

In other words, getting at least `k=2` pathway genes among our `n=8` was not at all unlikely if `K=50` out of all `N=100` genes are pathway genes.

More realistically, if the universe were larger, say, `N=20,000`, then the pathway genes would be much more rare. Then the p-value would be much smaller:

```{r}
sum(dhyper(x=2:8, m=50, n=19500, k=8))
```
