...
<div class="yt-embed">
  <div class="ratio">
    <iframe
      src="https://www.youtube.com/embed/rpKcmSqqm8g"
      title="Lecture video"
      frameborder="0"
...
When should you put `ggplot` option inside of `aes()` and when shouldn't you?

- Put it inside `aes()` if it depends on the data, i.e., if it varies form row to row of the data (like `aes(color=sex)`, which explicitly names a column of your data frame in order to color each observation by its particular `sex`)

- Don't put it inside of `aes()` if it's a fixed styling choice (like `color="blue"`, which would apply to every point, and which does not use hte name of a column in the data frame)
